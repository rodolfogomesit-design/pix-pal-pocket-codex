...
BEGIN
  ...
  IF _existing_user_id IS NOT NULL THEN
    ...
    -- Update profile with CPF and phone if not already set
    UPDATE profiles SET
      cpf = COALESCE(profiles.cpf, _cpf),
      telefone = COALESCE(profiles.telefone, _telefone),
      nome = COALESCE(NULLIF(profiles.nome, 'Usuário'), _nome)
  ...
  ELSE
    IF _senha IS NOT NULL AND LENGTH(_senha) >= 6 THEN
      ...
      -- The handle_new_user trigger creates the profile, now update it with CPF and phone
      UPDATE profiles SET
        cpf = _cpf,
        telefone = _telefone
