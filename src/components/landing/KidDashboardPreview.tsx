...
const KidDashboardPreview = () => {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      setActiveKidName((current) => (current === names[0] ? names[1] : names[0]));
    }, 5000);
        ...
        <AnimatePresence mode="wait">
          {activePreview === "inicio" && (
            <motion.div
              ...
              className="space-y-5"
            >
              <div className="rounded-[2rem] border border-border bg-card p-6 text-center shadow-xl">
                <p className="mb-1 text-sm text-muted-foreground">Meu saldo</p>
                <p className="font-display text-5xl font-extrabold text-primary">R$ 320,00</p>
                <div className="mt-4 flex justify-center gap-2 text-primary">
              ...
              <div>
                <p className="mb-3 font-display text-lg font-bold text-foreground dark:text-white">O que você quer fazer?</p>
              ...
              <div className="flex flex-wrap gap-2">
                <span className="rounded-full bg-muted px-3 py-1 text-xs text-muted-foreground dark:bg-slate-800 dark:text-slate-200">Limite: R$ 50/dia</span>
                <span className="rounded-full bg-muted px-3 py-1 text-xs text-muted-foreground dark:bg-slate-800 dark:text-slate-200">R$ 120,00 guardados</span>
              </div>
            </motion.div>
          )}

          {activePreview === "historico" && (
            <motion.div
              key="historico"
              initial={{ opacity: 0, x: -18 }}
              animate={{ opacity: 1, x: 0 }}
              ...
              className="space-y-3"
            >
              <p className="font-display text-lg font-bold text-foreground dark:text-white">Histórico</p>
              {[
                ["Mesada recebida", "+ R$ 25,00", "Hoje"],
                ["Guardado no cofrinho", "- R$ 10,00", "Ontem"],
                ["Pagamento para amigo", "- R$ 8,00", "Ontem"],
                ...
                <div key={title} className="rounded-2xl border border-border bg-card p-4 shadow-sm">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <p className="font-display font-bold text-primary">{amount}</p>
                  </div>
                </div>
              ))}
            </motion.div>
          ...
          {activePreview === "pagar" && (
            ...
            >
              <div className="rounded-[2rem] border border-border bg-card p-5 shadow-xl">
                <p className="font-display text-lg font-bold text-foreground">Pagar um amigo</p>
          ...
          {activePreview === "pix" && (
            ...
            >
              <div className="rounded-[2rem] border border-border bg-card p-5 shadow-xl">
                <p className="font-display text-lg font-bold text-foreground">Pagar com Pix</p>
                <div className="mt-4 space-y-3">
          ...
          {activePreview === "poupar" && (
            ...
            >
              <div className="rounded-[2rem] border border-border bg-card p-5 shadow-xl">
                <p className="font-display text-lg font-bold text-foreground">Poupar</p>
                <p className="mt-1 text-sm text-muted-foreground dark:text-slate-300">Guardar parte do saldo no cofrinho.</p>
                <div className="mt-4 h-3 overflow-hidden rounded-full bg-muted">
                  <div className="h-full w-[68%] rounded-full bg-[linear-gradient(90deg,#fb7185,#f59e0b,#22c55e,#38bdf8,#8b5cf6)]" />
                </div>
                <p className="mt-3 text-sm font-semibold text-primary">R$ 120,00 guardados</p>
              </div>
            </motion.div>
          )}

          {activePreview === "comissao" && (
            <motion.div
              key="comissao"
              initial={{ opacity: 0, x: -18 }}
              animate={{ opacity: 1, x: 0 }}
        ...
        <div className="flex items-center justify-around">
          {tabItems.map((tab) => (
            <button
              ...
              className={`flex flex-col items-center gap-1 rounded-2xl px-4 py-2 text-xs font-semibold ${
                activePreview === tab.id ? "bg-primary/10 text-primary" : "text-muted-foreground"
              }`}
            >
